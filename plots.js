...
let plot_satellites = function(d) {
    // Plot satellites as points in respective orbit level
    // ID = satellite name
    ...
    if (d.orbitClass == "LEO"){
        ...
        var dot = satelliteGroup.append("circle")
            .attr("class", "satPoint")
            .attr("id", d.satName)
        ...
        dot = satelliteGroup.append("circle")
            .attr("class", "satPoint")
            .attr("id", d.satName)
            .attr("cx", 0 + centerPoint)
            .attr("cy", 0 + centerPoint)
        ...
        dot = satelliteGroup.append("circle")
            ...
            .attr("cy", 0+ centerPoint)
            .attr("r", r)
            .attr("fill", color).style("opacity", 0);
        
        dot.transition()
            .duration(satSpeed)
            .attr("cx", radius * Math.cos(angle) + centerPoint)
            .attr("cy", radius * Math.sin(angle) + centerPoint).style("opacity", 1);
    }
...
let filterByType = function(){
    let angleData = [];
    let radiusData = [];
    let allSats = satelliteGroup.selectAll("circle").data(satData)
    
    // Update the colors based on use case 
    // Commericial: Green
    // Governemnt: Blue
    // Military: Red
    // Civil: White
    allSats.transition().duration(2000)
    ...
    .attr("cx", function(d){
        ...
        if (d.orbitClass == "GEO") { radius = 250 + (Math.random() * 30)}
        else if (d.orbitClass == "MEO") {radius = 200 + (Math.random() * 20)}
        else if (d.orbitClass == "LEO") { radius = 100 + (Math.random() * 80)}
        
        angleData.push(angle); radiusData.push(radius);
